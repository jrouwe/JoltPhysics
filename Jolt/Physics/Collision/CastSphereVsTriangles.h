...
public:
	...
	/// @param inShape Shape that contains this triangle
	...
	/// @param ioCollector The collector that receives the results.
									CastSphereVsTriangles(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, CastShapeCollector &ioCollector);
...
protected:
	...
	const Shape *					mSphereShape;						///< Sphere that is being cast
	const ShapeCastSettings &		mShapeCastSettings;
	const Shape *					mShape;								///< Shape that contains this triangle
