...
public:
	...
	inline void				SetAllowKinematicVsNonDynamic(bool inAllow)						{ JPH_ASSERT(IsRigidBody()); if (inAllow) mFlags.fetch_or(uint8(EFlags::AllowKinematicVsNonDynamic), memory_order_relaxed); else mFlags.fetch_and(uint8(~uint8(EFlags::AllowKinematicVsNonDynamic)), memory_order_relaxed); }

	/// Check if kinematic objects can generate contact points against other kinematic or static objects.
	inline bool				GetAllowKinematicVsNonDynamic() const							{ return (mFlags.load(memory_order_relaxed) & uint8(EFlags::AllowKinematicVsNonDynamic)) != 0; }
...
private:
	...
	enum class EFlags : uint8
	{
		...
		AllowKinematicVsNonDynamic	= 1 << 1,												///< If kinematic objects can generate contact points against other kinematic or static objects.
