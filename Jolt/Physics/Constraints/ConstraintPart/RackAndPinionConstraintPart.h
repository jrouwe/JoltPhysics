...
class RackAndPinionConstraintPart
{
	...
	JPH_INLINE bool				ApplyVelocityStep(Body &ioBody1, Body &ioBody2, float inLambda) const
	{
		...
		if (inLambda != 0.0f)
		{
			...
			// v' = v + M^-1 P
			ioBody1.GetMotionProperties()->AddAngularVelocityStep(inLambda * mInvI1_Axis);
			ioBody2.GetMotionProperties()->SubLinearVelocityStep(inLambda * mRatio_InvM2_Axis);
			return true;
		}
...
public:
	...
	inline void					CalculateConstraintProperties(const Body &inBody1, Vec3Arg inWorldSpaceHingeAxis, const Body &inBody2, Vec3Arg inWorldSpaceSliderAxis, float inRatio)
	{
		...
		// Calculate: I1^1 a
		mInvI1_Axis = inBody1.GetMotionProperties()->MultiplyWorldSpaceInverseInertiaByVector(inBody1.GetRotation(), inWorldSpaceHingeAxis);

		// Calculate: r/m2 b
		float inv_m2 = inBody2.GetMotionProperties()->GetInverseMass();
		mRatio_InvM2_Axis = inRatio * inv_m2 * inWorldSpaceSliderAxis;

		// K^-1 = 1 / (J M^-1 J^T) = 1 / (a^T I1^-1 a + 1/m2 * r^2)
		mEffectiveMass = 1.0f / (inWorldSpaceHingeAxis.Dot(mInvI1_Axis) + inv_m2 * Square(inRatio));
	}
	...
	inline bool					SolvePositionConstraint(Body &ioBody1, Body &ioBody2, float inC, float inBaumgarte) const
	{
		...
		if (inC != 0.0f)
		{
			...
			if (ioBody1.IsDynamic())
				ioBody1.AddRotationStep(lambda * mInvI1_Axis);
			if (ioBody2.IsDynamic())
				ioBody2.SubPositionStep(lambda * mRatio_InvM2_Axis);
			return true;
		}
