...
public:
	...
	virtual SoftBodyValidateResult	OnSoftBodyContactValidate([[maybe_unused]] const Body &inSoftBody, [[maybe_unused]] const Body &inOtherBody) { return SoftBodyValidateResult::AcceptContact; }

	/// Called after OnSoftBodyContactValidate accepted the contact to get the contact settings.
	/// This callback can be used to change the behavior of the collision response for all vertices.
	/// Note that this callback is called when all bodies are locked, so don't use any locking functions!
	/// @param inSoftBody The soft body that collided. It is safe to access this as the soft body is only updated on the current thread.
	/// @param inOtherBody The other body that collided. Note that accessing the position/orientation/velocity of inOtherBody may result in a race condition as other threads may be modifying the body at the same time.
	/// @param ioSettings The settings for all contact points that are generated by this collision.
	virtual void					GetSoftBodyContactSettings([[maybe_unused]] const Body &inSoftBody, [[maybe_unused]] const Body &inOtherBody, [[maybe_unused]] SoftBodyContactSettings &ioSettings) { /* Do nothing */ }
