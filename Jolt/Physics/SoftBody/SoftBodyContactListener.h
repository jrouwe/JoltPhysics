...
/// Return value for the OnSoftBodyContactValidate callback. Determines if the contact is being processed or not.
...
/// Note that you will receive a single callback for a soft body for performance reasons, this callback will apply to all vertices in the soft body
...
public:
	...
	/// Called whenever the soft body's aabox overlaps with another body's aabox (so receiving this callback doesn't tell if any over the vertices will collide).
	...
	/// @param inSoftBody The soft body that collided
	/// @param inOtherBody The other body that collided
	/// @param ioSettings The settings for all contact points that are generated by this collision
	/// @return Whether the contact should be processed or not (only AcceptContact and RejectContact are meaningful return values)
