...
public:
	...
	explicit						TempAllocatorImpl(uint64 inSize) :
	...
	virtual void *					Allocate(uint inSize) override
	{
		...
		{
			uint64 new_top = mTop + AlignUp(inSize, JPH_RVECTOR_ALIGNMENT);
			if (new_top > mSize)
			{
...
private:
	...
	uint64							mSize;							///< Size of the memory block
	uint64							mTop = 0;						///< End of currently allocated area
};
