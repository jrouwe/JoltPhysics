...
public:
	...
	JPH_INLINE float			operator () (uint inRow, uint inColumn) const			{ JPH_ASSERT(0 <= inRow && inRow < 4); JPH_ASSERT(0 <= inColumn && inColumn < 4); return mCol[inColumn].mF32[inRow]; }
	JPH_INLINE float &			operator () (uint inRow, uint inColumn)					{ JPH_ASSERT(0 <= inRow && inRow < 4); JPH_ASSERT(0 <= inColumn && inColumn < 4); return mCol[inColumn].mF32[inRow]; }
	...
	JPH_INLINE Vec3				GetColumn3(uint inCol) const							{ JPH_ASSERT(0 <= inCol && inCol < 4); return Vec3(mCol[inCol]); }
	JPH_INLINE void				SetColumn3(uint inCol, Vec3Arg inV)						{ JPH_ASSERT(0 <= inCol && inCol < 4); mCol[inCol] = Vec4(inV, inCol == 3? 1.0f : 0.0f); }
	JPH_INLINE Vec4				GetColumn4(uint inCol) const							{ JPH_ASSERT(0 <= inCol && inCol < 4); return mCol[inCol]; }
	JPH_INLINE void				SetColumn4(uint inCol, Vec4Arg inV)						{ JPH_ASSERT(0 <= inCol && inCol < 4); mCol[inCol] = inV; }
