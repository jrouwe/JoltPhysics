...
Quat Quat::operator * (QuatArg inRHS) const
{
	...
	__m128 m1 = _mm_mul_ps(bcab, zxyy);
#if defined(JPH_USE_FMADD)
	__m128 m3 = _mm_fmadd_ps(abca, wwwx, m1);
#else
	__m128 m2 = _mm_mul_ps(abca, wwwx);
...
#if defined(JPH_USE_FMADD)
	__m128 m6 = _mm_fmsub_ps(dddd, xyzw, m4);
#else
	__m128 m5 = _mm_mul_ps(dddd, xyzw);
	__m128 m6 = _mm_sub_ps(m5, m4);
#endif

	// [dw-ax-by-cz,dz+ay-bx+cw,dy-az+bw+cx,dx+aw+bz-cy]
	return Quat(Vec4(_mm_add_ps(m3, m6)));
#else
