...
Along with the readme's you'll find in this project, we also have documentation which is generated by [doxygen](https://www.doxygen.nl/index.html). This allows us to have the entire api crawled and have webpages generated for each of the classes that exists within jolt and be able to reference source code files. The benefit of using doxygen is that it allows us to write manual/wiki/tutorial files which can link directly to the source code and classes which allows users to explore how the project works internally much easier.

Doxygen also lets us write custom pages that are not automatically generated from the code, in our repository those files are included in the `Docs` directory. 

**To build the documentation locally run `doxygen Doxfile` at the root directory of this project. The `Doxfile` contains configuration for how doxygen.**

We also use a github actions to automatically run the `doxygen` command to compile the documentation, and then use github pages to host the files whenever a successfull merge request comes in.
