...
void TriangleShapeTest::Initialize()
{
	...
	Body &body1 = *mBodyInterface->CreateBody(BodyCreationSettings(triangle_shape, Vec3::sZero(), Quat::sIdentity(), EMotionType::Static, Layers::NON_MOVING));
	mBodyInterface->AddBody(body1.GetID(), EActivation::DontActivate);

	// Create a box above the triangle
	RefConst<Shape> box_shape = new BoxShape(Vec3(0.2f, 0.2f, 0.4f), 0.01f);
	Body &dynamic = *mBodyInterface->CreateBody(BodyCreationSettings(box_shape, Vec3(0, 5, 0), Quat::sIdentity(), EMotionType::Dynamic, Layers::MOVING));
