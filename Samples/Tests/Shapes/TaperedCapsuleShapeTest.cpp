...
void TaperedCapsuleShapeTest::Initialize() 
{
	...
	Body &body1 = *mBodyInterface->CreateBody(BodyCreationSettings(big_taperedcapsule, Vec3(0, 10, 0), Quat::sIdentity(), EMotionType::Dynamic, Layers::MOVING));
	mBodyInterface->AddBody(body1.GetID(), EActivation::Activate);

	// Tapered capsule on other outer sphere
	Body &body2 = *mBodyInterface->CreateBody(BodyCreationSettings(big_taperedcapsule2, Vec3(10, 10, 0), Quat::sIdentity(), EMotionType::Dynamic, Layers::MOVING));
	mBodyInterface->AddBody(body2.GetID(), EActivation::Activate);

	// Tapered capsule on side
	Body &body3 = *mBodyInterface->CreateBody(BodyCreationSettings(big_taperedcapsule, Vec3(20, 10, 0), Quat::sRotation(Vec3::sAxisX(), 0.5f * JPH_PI), EMotionType::Dynamic, Layers::MOVING));
