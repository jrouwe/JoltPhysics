...
void DampingTest::Initialize()
{
	...
	for (int i = 0; i <= 10; ++i)
	{
		...
		SetBodyLabel(body.GetID(), StringFormat("Linear damping: %.1f", body.GetMotionProperties()->GetLinearDamping()));
	}
	...
	for (int i = 0; i <= 10; ++i)
	{
		...
		mBodyInterface->AddBody(body.GetID(), EActivation::Activate);
		SetBodyLabel(body.GetID(), StringFormat("Angular damping: %.1f", body.GetMotionProperties()->GetAngularDamping()));
	}
}
